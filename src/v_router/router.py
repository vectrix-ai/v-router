...
class Router:
        ...
        if provider_name not in self._provider_instances:
            ...
            if provider_name == "vertexai":
                ...
                if self.primary_config.model_name.startswith("claude"):
                    self._provider_instances[provider_name] = AnthropicVertexProvider(**self.provider_kwargs)
                else:
                    self._provider_instances[provider_name] = GoogleVertexProvider(**self.provider_kwargs)
            else:
                self._provider_instances[provider_name] = provider_class(**self.provider_kwargs)
    ...
    async def route_request(
        ...
        **kwargs
        ...
        try:
            logger.info(f"Trying primary model: {self.primary_config.model_name} on {self.primary_config.provider}")
        ...
        for backup_model in self.primary_config.get_ordered_backup_models():
            try:
                logger.info(f"Trying backup model: {backup_model.model_name} on {backup_model.provider}")
                return await self._try_provider(
                    backup_model,
                    messages,
                    **kwargs
        ...
        if self.primary_config.try_other_providers:
            alternative_providers = self._get_alternative_providers(
                ...
                exclude=[self.primary_config.provider]
            )
            
            for alt_provider in alternative_providers:
                try:
                    logger.info(f"Trying alternative provider: {self.primary_config.model_name} on {alt_provider}")
                    alt_config = LLM(
